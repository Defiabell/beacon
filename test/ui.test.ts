...
import type { Overview, ProjectDetail, MatrixData, MatrixCoverageRow, PostWithMetrics, ProjectSummary, SurfaceBreakdown } from "../src/api/public";
import type { Todo } from "../src/types";
import type { EventImpact } from "../src/impact/attribute";

// Every Overview literal below needs a SurfaceBreakdown; only the two tests
// that assert on the section itself care about its contents, so the rest share
// this empty one.
const NO_SURFACES: SurfaceBreakdown = { surfaces: [], unclassified: [], since: null };
...
describe("renderOverview", () => {
  ...
  it("doesn't crash on an all-empty Overview", () => {
    const overview: Overview = { projects: [], topTodos: [], suggestions: [], sources: [], sitePv7d: 0, surfaces: NO_SURFACES, workers: [], sites: [] };
...
describe("header login/logout link (navHeader)", () => {
  it("renderOverview shows a 登录 link when logged out", () => {
    const html = renderOverview({ projects: [], topTodos: [], suggestions: [], sources: [], sitePv7d: 0, surfaces: NO_SURFACES, workers: [], sites: [] }, false);
  ...
  it("renderOverview shows a 登出 form (not a link) when logged in", () => {
    const html = renderOverview({ projects: [], topTodos: [], suggestions: [], sources: [], sitePv7d: 0, surfaces: NO_SURFACES, workers: [], sites: [] }, true);
